          ...
          {pg === "register" && (<>
            {viewQR ? (
              ...
              <div style={{ maxWidth: "520px", margin: "0 auto" }}><button onClick={() => setViewQR(null)} style={{ ...BTN("out"), marginBottom: "14px", padding: "10px", fontSize: "12px" }}>← Back</button><div className="card" style={{ border: `1px solid ${qs.color === "green" ? P.ok + "40" : qs.color === "red" ? P.err + "40" : P.acc + "40"}`, textAlign: "center", padding: "32px" }}><div style={LBL}>Your Iftar Pass</div><div style={{ fontSize: "20px", fontWeight: "800", color: P.pri, margin: "8px 0 4px", fontFamily: "'Playfair Display'" }}>{viewQR.name}</div><div style={{ fontSize: "12px", color: P.acc, fontWeight: "600", marginBottom: "16px" }}>For: {viewQR.iftarDate || viewQR.date}</div>
                ...
                {!viewQR.used && viewQR.date === now.toDateString() && now.getHours() < cfg.regEndHour && (<div style={{ marginTop: "16px" }}><button onClick={() => { if (window.confirm("Are you sure you want to cancel this registration? This cannot be undone.")) { const vid = viewQR.id; setViewQR(null); setRegs(regs.filter(r => r.id !== vid)); showToast("Registration cancelled. You won't receive a strike for this.", "ok"); } }} style={{ width: "100%", padding: "14px", borderRadius: "12px", border: `2px solid ${P.err}30`, background: "#fef2f2", color: P.err, fontWeight: "700", fontSize: "14px", cursor: "pointer" }}>❌ Cancel Registration</button><div style={{ fontSize: "11px", color: P.mut, marginTop: "8px", textAlign: "center" }}>Cancel before {fH(cfg.regEndHour)} to avoid a no-show strike</div></div>)}
